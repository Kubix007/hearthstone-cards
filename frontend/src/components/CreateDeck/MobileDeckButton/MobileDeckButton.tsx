...
import * as Styles from "./MobileDeckButton.styles";
import * as Types from "./MobileDeckButton.types";

const MobileDeckButton = ({ setIsShowed }: Types.IProps) => {
  const { metadata } = useSelector((state: RootState) => state.metadata);
  const { deck, isSelected } = useSelector(
  ...
  return (
    <Styles.Container onClick={() => setIsShowed(true)}>
      <Styles.MobileDeckBackground />
      <Styles.ClassImage $backgroundImg={$backgroundImg} />
      <Styles.CardCount>{deck.cardCount}/30</Styles.CardCount>
    </Styles.Container>
  );
};
