...
import * as Styles from "./DeckListClassBackground.styles";
import DeckListGameModeIcon from "../DeckListGameModeIcon";

const DeckListClassBackground = () => {
  let backgroundImg = "";
  ...
  if (isSelected) {
    switch (deck.hero?.name) {
      ...
      case "Wojownik":
        ...
        pageHeaderTitleText = `Talia: ${deck.hero?.name} (${deck.format})`;
        break;
  ...
  return (
    <Styles.DeckListBackground backgroundImg={backgroundImg}>
      ...
      <Styles.DetailsContainer>
        ...
        <Styles.DeckListCardsAmount>
          {deck.cardCount}/30
        </Styles.DeckListCardsAmount>
      </Styles.DetailsContainer>
    </Styles.DeckListBackground>
  );
};
